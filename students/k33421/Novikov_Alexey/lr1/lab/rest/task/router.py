...
from lab.db.models import Task as TaskModel, TaskLink as TaskLinkModel, Sprint as SprintModel
from lab.rest.task.schemas import (
    TaskResponse,
    TaskBodySchema,
    MessageResponse,
...
def add_task(task_body: TaskBodySchema, session: DatabaseSession, response: Response):
    ...
    response = TaskResponse.model_validate(task_model)

    session.commit()

    return TaskDataResponse(status=200, data=response)
